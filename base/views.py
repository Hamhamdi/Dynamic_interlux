...
from base.form import  AdminForm, DynForm,  RegistrationForm, AccountAuthenticationForm, ResCa, ResFin, ResInf, Rl1Form, Rl2Form
from django.db.models.functions import Upper

# from base.form import RoleForm
from .models import  Base, ConTable,   GeneralDTable
...
def General_dyn(request):
    ...
    # for x in general_dyn:
    #     general_dyn_list.append(x)


  

    # fields_name = DynGt._meta.get_fields(include_parents=True, include_hidden=False)

    # all_fields = [f.name for f in DynGt._meta.fields]
...
def manage_dyanmic(request):
    ...
    #trying to pass a list filled with fields according to user etape
    emptylist = []

    # for p in ConTable.objects.values_list('champ', flat=True).filter(etape = request.user.etape):
    #     emptylist.append(p)

    p = ConTable.objects.filter(etape = request.user.etape)
     
    
    #passing the list to the form
    formset = DynForm(x = p)
    # for i in p :
        # if i.type == 'DATE':
            # DynForm = modelformset_factory(BaseAdmintable,form=DynForm ,widgets={i.champ: DateInput(attrs={'type':'date'})})
   # formset.set_form_fields(x = emptylist)
    # DynForm.Meta(x = emptylist)

    if request.POST:
        formset = DynForm(request.post)
        if formset.is_valid():
